...
Base de datos	MySQL (relacional)	El dominio tiene relaciones fuertes entre entidades (suscriptor–suscripción–pedido–pago) que se benefician de claves foráneas e integridad transaccional (ACID). Además, es el motor que el equipo ya utilizó en cuatrimestres anteriores.
...
Riesgos identificados: ausencia de framework en el frontend a medida que crezcan las pantallas (mitigado con una estructura de carpetas y componentes reutilizables desde el inicio), y dependencia de un modelo relacional si en el futuro se necesitara almacenar contenido más flexible (por ejemplo, para el Club de Lectura Virtual).
...
Notificaciones automáticas por email ante cambios de estado del pedido.
Panel simple de reportes para las administradoras (ej. cajas despachadas por temática).
